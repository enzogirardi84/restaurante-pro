"""
views/caja.py - Terminal de Caja: cierre de mesas, pago y ticket descargable.
"""
...
from components.tickets import imprimir_si_hay_impresora, mostrar_ticket_streamlit
...
def _cargar_items_mesa(id_mesa: int) -> list:
    ...
    try:
        cur = conn.execute(
            """
            SELECT pm.nombre,
                   SUM(pd.cantidad - COALESCE(pd.cantidad_anulada, 0)) AS cant,
                   pm.precio_venta
...
def _seleccionar() -> None:
    st.markdown("<h1 style='text-align:center'>&#x1F9FE; Terminal de Caja</h1>", unsafe_allow_html=True)
    ...
    for i in range(0, len(mesas), cols_por_fila):
        ...
        for j, mesa in enumerate(mesas[i : i + cols_por_fila]):
            with cols[j]:
                st.markdown(
                    f"<div style='background:#2e7d32;border-radius:8px;padding:1.4rem;"
                    f"text-align:center;color:white;font-size:1.7rem;font-weight:700'>"
                    f"&#x1FA91; Mesa {mesa['numero_mesa']}</div>",
                    unsafe_allow_html=True,
                )
                if st.button("Ver cuenta", key=f"caja_ver_{mesa['id_mesa']}", use_container_width=True):
                    st.session_state.caja_mesa = dict(mesa)
...
def _cuenta() -> None:
    ...
    st.markdown(f"<h2>&#x1F9FE; Cuenta - Mesa #{mesa['numero_mesa']}</h2>", unsafe_allow_html=True)
    ...
    subtotal = 0.0
    for item in items:
        importe = float(item["cant"] or 0) * float(item["precio_venta"] or 0)
        subtotal += importe
        st.markdown(
            f"<div style='display:flex;justify-content:space-between;"
            f"padding:0.5rem 0.2rem;border-bottom:1px solid #eee'>"
            f"<span><b>{item['cant']} x</b> {escape(str(item['nombre']))}</span>"
    ...
    if metodo == "Efectivo":
        ...
        with col_ef2:
            ...
            color = "#388e3c" if vuelto >= 0 else "#c62828"
            label = "Vuelto" if vuelto >= 0 else "Faltan"
            st.markdown(
                f"<div style='background:{color};color:white;padding:1rem;"
                f"border-radius:8px;text-align:center;margin-top:1.6rem;"
                f"font-size:1.2rem;font-weight:700'>{label}<br>${abs(vuelto):,.0f}</div>",
                unsafe_allow_html=True,
            )
    elif metodo == "Tarjeta":
    ...
    puede_cobrar = (metodo == "Efectivo" and monto_recibido >= total) or metodo in ("Tarjeta", "Digital")
...
def _mostrar_pantalla_ticket() -> None:
    ...
    st.markdown(
        "<p style='text-align:center;color:#555'>A continuacion podes visualizar, imprimir o descargar el ticket.</p>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown("---")
    if st.button("Nueva operacion", type="primary", use_container_width=True):
        st.session_state.caja_mesa = None
        st.session_state.caja_cobrado = False
        st.session_state.caja_ticket_id = None
        st.rerun()
...
def _ejecutar_cierre(mesa: dict, total: float, metodo: str) -> None:
    """Marca todos los pedidos activos como cobrados y libera la mesa."""
    ...
    try:
        ...
        conn.execute("UPDATE mesas SET estado = 'libre' WHERE id_mesa = ?", (mesa["id_mesa"],))
        conn.commit()
        _cargar_mesas_ocupadas.clear()
        _cargar_items_mesa.clear()
    except Exception as exc:
        conn.rollback()
        st.error(f"Error al cerrar la cuenta: {exc}")
        return
    finally:
        conn.close()
