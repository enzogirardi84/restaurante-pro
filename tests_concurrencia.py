...
from database import execute_query, get_connection, get_db_type
...
def rearmar_db():
    ...
    try:
        ...
        conn.execute("INSERT OR IGNORE INTO usuarios (nombre, apellido, rol, pin) VALUES ('Test', 'Mozo', 'mozo', '9999')")
        conn.commit()
...
def hilo_mozo(hilo_id: int):
    ...
    for intento in range(PEDIDOS_POR_HILO):
        try:
            ...
            conn.execute(f"UPDATE mesas SET estado = 'ocupada' WHERE id_mesa = {ph}", (mid,))
...
def hilo_cocina(hilo_id: int):
    ...
    for _ in range(PEDIDOS_POR_HILO * 2):
        try:
            ...
            if ok:
                conn.execute(f"UPDATE pedidos_cabecera SET estado_comanda = 'listo' WHERE id_pedido = {ph}", (pid,))
                conn.execute("COMMIT")
...
def hilo_caja(hilo_id: int):
    ...
    for _ in range(PEDIDOS_POR_HILO):
        try:
            ...
            conn.execute(f"""UPDATE pedidos_cabecera SET estado_comanda = 'cobrado', medio_pago = 'efectivo',
                             total_cobrado = {ph}, fecha_cobro = datetime('now','localtime') WHERE id_pedido = {ph}""", (total, pid))
